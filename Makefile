...
ifeq (${OS},Windows_NT)
  ...
  ifeq (${CC},icl)    # Intel Windows compiler
    CFLAGS = -DSPECIALIZE_RELPROD -DSPECIALIZE_OR -DSPECIALIZE_AND /O2 /Ob2 $(EXTRA_CFLAGS)
  ...
  CLASSPATH = .:jdd.jar
  CFLAGS = -DSPECIALIZE_RELPROD -DSPECIALIZE_AND -DSPECIALIZE_OR -O2 -fomit-frame-pointer $(EXTRA_CFLAGS)
...
$(JAR_NAME): $(BUDDY_CLASSFILE) $(CUDD_CLASSFILE) $(CAL_CLASSFILE) $(JAVA_CLASSFILES) $(EXAMPLE_CLASSFILES)
